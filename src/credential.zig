...
pub fn writeCredential(allocator: mem.Allocator, username: []const u8, password: []const u8) !void {
    const target_utf16: [:0]u16 = try unicode.utf8ToUtf16LeAllocZ(allocator, "TERMSRV/localhost");
    defer allocator.free(target_utf16);

    const username_utf16: [:0]u16 = try unicode.utf8ToUtf16LeAllocZ(allocator, username);
    ...
    credential.CredentialBlob = @ptrCast(&password_utf16[0]);
